...
class RevisionUpdater
        ...
        def Execute(data)  # UpdaterData
            ...
            # So for now, UNCHANGE the name of a rev if a user changes it!
            # Also nope! revision description is a read-only parameter.
            # Create a new parameter with the new rev name, copy all the sheet settings from the old name param to the new one,
            # and delete the old param from the fields.
            ...
            changed_rev_ids.each do |changed_rev_id|
                ...
                if old_rev_name_param
                    ...
                    remove_field_from_sheet_list( sched, old_rev_name, ParameterType.YesNo, sub_txn )
                end
            end

            # There's no point in this.  And merging revs doesn't trigger it.
        ...
        def GetAdditionalInformation()
            return "Revision updater: adds/changes/deletes a field in Sheet Log with the same name as the Revision (field is also used by Sheet Revision Change updater."
        end
        ...
        #----------------
        def GetUpdaterName()
            return "Revision Updater"
        end
end

#------------------
def revisions_register_updater( app, doc = nil )
    begin
...
#---------------
def revisions_unregister_updater( app, updater = nil )
...
if $is_revitrubyshell
    ...
    revisions_register_updater( _app.Application, _app.active_ui_document.document )
    $sheets_updater = sheets_register_updater( _app, _app.Application, _app.active_ui_document.document )
    #revisions_unregister_updater( _app.Application )
    #sheets_unregister_updater( _app, _app.Application, $sheets_updater )
end
